...
public class Armstrong {
    ...
    public static List<Long> getArmstrongListParallel(final long endIndex) throws InterruptedException, ExecutionException {
        ...
        if (endDegree > 12) {
            ...
            futures.add(executorService.submit(() -> Armstrong.getArmstrongListByDegree(endIndex, 1, 12)));


            for (int i = 13; i <= endDegree; i++) {
                final int degree = i;
                futures.add(executorService.submit(() -> Armstrong.getArmstrongListByDegree(endIndex, degree, degree)));
            }
    ...
    private static long getNextNum(long number) {
        ...
        if (number++ % 10 != 9)
            ...
            StringBuilder sbNumber = new StringBuilder(Long.toString(number));
    ...
    private static long getArmstrongNumber(long N, int degree) {
        ...
        long armSum = getArmstrongSum(N, degree);
        return (armSum != -1 && isArmstrongNumber(armSum) && isDegreeNumber(armSum, degree)) ? armSum : -1;
    }
    ...
    private static boolean isDegreeNumber(long N, int inputDegree) {
        ...
        return numberDegree == inputDegree;
